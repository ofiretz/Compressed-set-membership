...
public class Trie<T> implements Serializable{
	 ...
	 */
	public void insertString(String s, T item, ArrayList<String> list){
		...
		if(node.item != null){ // We need to union two trees
			if(node.item instanceof Trie<?>)
				insertAll((Trie<String>)node.item, list);	
		}
		else{
			node.setItem(item);
		}
	...
	// Insert all strings in list to the specified trie
	public void insertAll(Trie<String> trie, ArrayList<String> list){
		int cnt = list.size();
		
		while(cnt > 0){
			trie.insertString(list.remove(0), "null", null);
			cnt--;
		}
	}
	
	public void insertHammingPair(Pair p){
		String bvs = p.getBVS();
		ArrayList<String> list = SetDistanceProvider.create1HammingSet(p.getSecond());
		
		if(this.search(bvs) == null){
			Trie<String> trie = new Trie<String>();
			insertAll(trie, list);
			this.insertString(bvs, (T) trie, null);
			
		}
		else{ // bvs is already in the trie
			this.insertString(bvs, null, list);
		}
		
	}
	
	public void insertPlus1Pair(Pair p){
		String bvs = p.getBVS();
		ArrayList<String> list = SetDistanceProvider.create1InsertionSet(p.getSecond());
	...
	public void insertMinus1Pair(Pair p){
		String bvs = p.getBVS();
		ArrayList<String> list = SetDistanceProvider.create1DelitionSet(p.getSecond());
		...
		if(p.getLast()){
			bvs = p.getBVS(p.getThird());
			list.clear();
			list.add(p.getFourth());
			if(this.search(bvs) == null){
				Trie<String> trie = new Trie<String>();
				insertAll(trie, list);
				this.insertString(bvs, (T) trie, null);
	...
	public Boolean pairSearch(Pair p){
		String s = p.getFirst();
		char[] cArray = s.toCharArray();
		TrieNode node = root, child;
		for (char c: cArray){
		...
		try{
			return pairSearchHelp(node.item, p);			
		}catch(NullPointerException e){
			return false;
		}
	}
	
	public Boolean pairSearchHelp(T trie, Pair p){
		if(trie == null)
			return false;

		if(((Trie<String>) trie).search2(p.getSecond()).equals("null"))
			return true;
